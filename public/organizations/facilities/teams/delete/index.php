...
if (!isset($_GET['id'])) {
    header('Location: /organizations/facilities/teams?error=No team specified');
    exit;
}
...
try {
    ...
    if ($isPermanent) {
        ...
        if ($success) {
            header('Location: /organizations/facilities/teams?success=Facility team permanently deleted');
        } else {
        ...
        if ($success) {
            header('Location: /organizations/facilities/teams?success=Facility team moved to trash');
        } else {
            throw new Exception("Failed to delete facility team");
        }
    }
} catch (Exception $e) {
    header('Location: /organizations/facilities/teams?error=' . urlencode($e->getMessage()));
}
