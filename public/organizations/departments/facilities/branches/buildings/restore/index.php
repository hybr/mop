...
$id = $_GET["id"];
try {
    if (!$buildingRepo->isSuperAdmin($user->getEmail())) { throw new \Exception("Only Super Admin can restore buildings"); }
    $buildingRepo->restore($id, $user->getEmail());
    header("Location: /organizations/departments/facilities/branches/buildings/?success=Building restored");
} catch (Exception $e) {
