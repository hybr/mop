...
if (!$deptRepo->isSuperAdmin($user->getEmail())) {
    header('Location: /organizations/departments/?error=' . urlencode('Only Super Admin can restore departments'));
    exit;
}
...
try {
    ...
    if ($success) {
        header('Location: /organizations/departments/?success=' . urlencode('Department restored successfully'));
    } else {
