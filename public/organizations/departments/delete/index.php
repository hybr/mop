...
if (!$deptRepo->canDelete($user->getEmail())) {
    header('Location: /organizations/departments/?error=' . urlencode('Only Super Admin can delete departments'));
    exit;
}
...
try {
    ...
    if ($success) {
        header('Location: /organizations/departments/?success=' . urlencode('Department "' . $deptName . '" deleted successfully'));
    } else {
        throw new Exception('Failed to delete department');
    }

} catch (Exception $e) {
    header('Location: /organizations/departments/?error=' . urlencode($e->getMessage()));
}
exit;