...
class OrganizationDepartmentRepository {
    private $db;
    private $tableName = 'organization_departments';
     ...
     */
    public function create(OrganizationDepartment $dept, $userId, $userEmail) {
        // TODO: In production, check if user has "Human Resource > Structure > Manager" position
        // For now, allow any authenticated user to create

        // Check code uniqueness
        if ($this->codeExists($dept->getCode())) {
            throw new \Exception("Department code '{$dept->getCode()}' is already taken. Please choose another.");
        }
        ...
        throw new \Exception("Failed to create department: " . json_encode($response['data'] ?? 'Unknown error'));
    }

    /**
     * Find department by ID (any user can view)
     * Per permissions.md: All users can view organization departments
     */
    public function findById($id) {
        if ($this->db->getDriver() === 'sqlite') {
            ...
            if ($data) {
                return new OrganizationDepartment($data);
            }
        } else {
            $response = $this->db->request('GET', $this->tableName . '?id=eq.' . $id . '&deleted_at=is.null');

            if ($response['success'] && !empty($response['data'])) {
                return new OrganizationDepartment($response['data'][0]);
            }
        }
    ...
    /**
     * Get all active departments (non-deleted)
     * Per permissions.md: All users can view organization departments
     */
    public function findAll($limit = 100, $offset = 0) {
        if ($this->db->getDriver() === 'sqlite') {
            ...
            foreach ($data as $deptData) {
                $departments[] = new OrganizationDepartment($deptData);
            }
            return $departments;
        } else {
     ...
     */
    public function update(OrganizationDepartment $dept, $userId, $userEmail) {
        if (!$dept->getId()) {
        ...
        if (!$this->isSuperAdmin($userEmail)) {
            throw new \Exception("Only Super Admin can update departments");
        }
    ...
    public function softDelete($id, $userId, $userEmail) {
        // Check Super Admin permission
        if (!$this->isSuperAdmin($userEmail)) {
            throw new \Exception("Only Super Admin can delete departments");
        }
    ...
    /**
     * Get deleted departments (for trash/restore functionality)
     * Per permissions.md: Only Super Admin can view deleted
     */
    public function findDeleted($userEmail, $limit = 100) {
        // Check Super Admin permission
        if (!$this->isSuperAdmin($userEmail)) {
            throw new \Exception("Only Super Admin can view deleted departments");
        }
    ...
    public function restore($id, $userEmail) {
        // Check Super Admin permission
        if (!$this->isSuperAdmin($userEmail)) {
            throw new \Exception("Only Super Admin can restore departments");
        }
    ...
    /**
     * Get departments as options for dropdown (for foreign key usage)
