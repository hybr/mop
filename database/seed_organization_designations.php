...
use App\Classes\OrganizationDesignation;
use App\Classes\OrganizationDesignationRepository;

$repo = new OrganizationDesignationRepository();

// Super Admin user ID for seeding
$superAdminUserId = 'sharma.yogesh.1234@gmail.com';
$superAdminEmail = 'sharma.yogesh.1234@gmail.com';

// Common designations across industries
$designations = [
    // Entry Level (Level 1)
...
foreach ($designations as $designationData) {
    try {
        ...
        if ($repo->codeExists($designationData['code'])) {
            echo "⚠️  Skipping '{$designationData['name']}' (code: {$designationData['code']}) - already exists\n";
            continue;
        }

        $designation = new OrganizationDesignation();
        $designation->setName($designationData['name']);
        $designation->setCode($designationData['code']);
        $designation->setLevel($designationData['level']);
        $designation->setSortOrder($designationData['sort_order']);
        $designation->setIsActive(1);

        $repo->create($designation, $superAdminUserId, $superAdminEmail);
        echo "✅ Created: {$designationData['name']} ({$designationData['code']}) - Level {$designationData['level']}\n";
        $successCount++;

    } catch (Exception $e) {
        echo "❌ Error creating '{$designationData['name']}': " . $e->getMessage() . "\n";
        $errorCount++;
    }
}
